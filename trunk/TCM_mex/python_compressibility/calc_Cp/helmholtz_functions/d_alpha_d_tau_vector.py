def d_alpha_d_tau_vector(tau,delta,parameters):
	...
	t1 = N_i_m[power_terms_wo_exp][:]*t_m[power_terms_wo_exp][:]*pow(delta_m[power_terms_wo_exp][:],d_m[power_terms_wo_exp][:])*pow(tau_m[power_terms_wo_exp][:],t_m[power_terms_wo_exp][:] - 1.0)
        t1_v=t1.sum(axis=0)
	
        t2 = N_i_m[power_terms_w_exp][:]*t_m[power_terms_w_exp][:]*pow(delta_m[power_terms_w_exp][:],d_m[power_terms_w_exp][:])*pow(tau_m[power_terms_w_exp][:],t_m[power_terms_w_exp][:] - 1.0)*exp(-pow(delta_m[power_terms_w_exp][:],c_m[power_terms_w_exp][:]))
        t2_v=t2.sum(axis=0)    
        ...
        if(parameters['n_gaussian_terms']>0):
	        ...
	        dummy,eps_m=meshgrid(delta,parameters['D_i'])
                ...
                p2 = t_m[gaussian_terms][:]/tau_m[gaussian_terms][:] - 2.0*beta_m[gaussian_terms][:]*(tau_m[gaussian_terms][:] - gamma_m[gaussian_terms][:])
                t3 = N_i_m[gaussian_terms][:]*pow(delta_m[gaussian_terms][:],d_m[gaussian_terms][:])*pow(tau_m[gaussian_terms][:],t_m[gaussian_terms][:])*exp(p1)*p2
                t3_v=t3.sum(axis=0)
        else:
                t3_v[0]=0.0
        ...
        if(parameters['n_critical_terms']>0):
	        ...
	        dummy,D_m=meshgrid(delta,parameters['RES_D'])
                ...
                DELTA = THETA*THETA + B_m[critical_terms][:]*pow((delta_m[critical_terms][:] - 1.0)*(delta_m[critical_terms][:] - 1.0),a_m[critical_terms][:])
                ...
                D_DELTA_bi_tau = -2.0*THETA*b_m[critical_terms][:]*pow(DELTA,b_m[critical_terms][:] - 1.0)
                D_PSI_tau = -2.0*D_m[critical_terms][:]*(tau_m[critical_terms][:] - 1.0)*PSI
                t4 = N_i_m[critical_terms][:]*delta_m[critical_terms][:]*(D_DELTA_bi_tau*PSI + pow(DELTA,b_m[critical_terms][:])*D_PSI_tau)
                t4_v=t4.sum(axis=0)
        else:
                t4_v[0]=0.0
