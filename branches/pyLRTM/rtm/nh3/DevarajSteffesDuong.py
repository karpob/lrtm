...
def NH3H2HeH2OModel(f,T,P,H2mr,Hemr,H2Omr,NH3mr):
    ...
    k=1.38*10e-23                      #boltzmann's in J/K or N.m/K
    ...
    Bb=np.power(np.power(f_matrix,2)-np.power(f_matrix,2),2)
    ...
    gH2_v2=np.linspace(1.4,1.4,length(fo_v2)).T
    gHe_v2=np.linspace(0.68,0.68,length(fo_v2)).T
    gNH3_v2=np.linspace(9.5,9.5,length(fo_v2)).T
    ...
    alpha_opdep=np.sum(br_alpha_matrix,axis=0)+np.sum(alpha_rot,axis=0)+np.sum(alpha_v2,axis=0)
