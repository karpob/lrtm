%function vvw_alpha=alphashape(catalog,f,PH2,PHe,PPH3,P,T);% Revised 9/21/99
...
%PH2=input('Enter % H2: ')/100;
%PHe=input('Enter % He: ')/100;
PPH3=input('Enter mixing ratio (q) PH3: ');
P=input('Enter total pressure (bars); ');
T=input('Enter Temperature (K): ');
%Fstart=input('Enter starting frequency (GHz): ');
%Fstop=input('Enter stopping frequency (GHz): ');
%Fstep=input('Enter frequency step (GHz): ');

%f=Fstart:Fstep:Fstop;			% Frequency Range Being Modeled
f=2.3;
PH2=0.87;
PHe=0.13;
%PPH3=PPH3/100;
...
vvw_alpha=vvw_alpha_neper.*NepertodB;
alpha=vvw_alpha
